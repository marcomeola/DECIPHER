...
\item Functions for phylogenetics with a set of sequences:
...
\begin{enumerate} \item \Rfunction{AlignProfiles} \item \Rfunction{AlignSeqs} \item \Rfunction{AlignTranslation} \item \Rfunction{MaskAlignment} \end{enumerate}
...
BrowseSequences(subseq(dna, 140, 240))
@

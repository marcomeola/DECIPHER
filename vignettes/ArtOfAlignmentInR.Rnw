...
The \Rfunction{AlignSeqs} uses a guide tree to decide which order to align pairs of sequence profiles.  The \Rfunarg{guideTree} input is a \Rclass{data.frame} with the grouping of each sequence at increasing levels of dissimilarity between the groups.  By default this guide tree is generated directly from the sequences using the order of shared k-mers (the argument \Rfunarg{guideTree} is \code{NULL}).  However, in some circumstances it may be desirable to provide a guide tree as input.

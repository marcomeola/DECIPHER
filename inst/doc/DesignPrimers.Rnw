...
\subsection{Installing UNAFold}

The program UNAFold (\url{http://mfold.rna.albany.edu/?q=DINAMelt/software}) is used to predict hybridization efficiency and must be installed in a location accessible by the system.  For example, the following code should print the installed UNAFold version when executed from the \term{R} console:
...
At this point we need to define groups of related sequences in the database we just created.  In this case we wish to define a group for the sequences of each species of \textit{Streptomyces}.  In order to accomplish this we make use of the information contained in the description of each FASTA sequence record that we just imported.
...
<<expr9,eval=FALSE>>=
sqrt(mean(primers[,"forward_efficiency"],
		na.rm=TRUE)*
	mean(primers[,"reverse_efficiency"],
		na.rm=TRUE))*100
@
...
1 avermitilis           242           329           88                   247
2 avermitilis           244           329           86                   249
3 avermitilis           243           329           87                   248
4 avermitilis           245           329           85                   250
5 avermitilis           250           329           80                   255
...
1  -0.00032....         0 CGTTGATTATTCGGCACACTCGAC             <NA>             <NA>
2  -0.00032....         0      TATTCGGCACACTCGACCT             <NA>             <NA>
3  -0.00032....         0    GATTATTCGGCACACTCGACC             <NA>             <NA>
4  -0.00032....         0        TCGGCACACTCGACCTG             <NA>             <NA>
5  -0.00032....         0      GCACACTCGACCTGCTCAG             <NA>             <NA>
  forward_primer.4   reverse_primer.1 reverse_primer.2 reverse_primer.3 reverse_primer.4
1             <NA> GCCCTCCCATGTTCGCTT             <NA>             <NA>             <NA>
2             <NA> GCCCTCCCATGTTCGCTT             <NA>             <NA>             <NA>
3             <NA> GCCCTCCCATGTTCGCTT             <NA>             <NA>             <NA>
4             <NA> GCCCTCCCATGTTCGCTT             <NA>             <NA>             <NA>
5             <NA> GCCCTCCCATGTTCGCTT             <NA>             <NA>             <NA>
...
<<expr12,eval=TRUE,fig=TRUE,height=7,width=8>>=
temp_range <- 60:75
ps <- c("CGTTGATTATTCGGCACACTCGAC", "GCCCTCCCATGTTCGCTT") # forward and reverse
f <- function(temp) {
...
names(amplicon) <- desc
amplicon <- unique(amplicon)
# move the target group to the top
w <- which(names(amplicon)=="avermitilis")
amplicon <- c(amplicon[w], amplicon[-w])
@

Then we can use the function \Rfunction{BrowseSequences} to visualize the target site for each primer in the amplicon.  This command will open a new window in the default web browser.  The top sequence shows the target (\textit{S. avermitilis}), and each primer's target site is colored.  It is clear why this target site was chosen, because all other \textit{Streptomyces} species have multiple mismatches to both the forward and reverse primers.  The non-target mismatches, especially those located nearest the 3' end (towards the center of the amplicon), will act to lower both hybridization and elongation efficiency of non-target amplification.

<<expr14,eval=FALSE>>=
BrowseSequences(amplicon, color=c(4, 27, 67, 89))
@
